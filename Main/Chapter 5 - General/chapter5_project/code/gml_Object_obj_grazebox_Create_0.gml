grazetimer = 0;
x_offset = 0;
grazetpfactor = 1;
grazetimefactor = 1;
grazesizefactor = 1;
grazetpfactor += (scr_armorcheck_equipped_party(15) * 0.15);
grazetpfactor += (scr_armorcheck_equipped_party(17) * 0.25);
grazetpfactor += (scr_armorcheck_equipped_party(24) * 0.05);
grazetimefactor += (scr_armorcheck_equipped_party(14) * 0.3);
grazesizefactor += (scr_armorcheck_equipped_party(3) * 0.2);
grazesizefactor += (scr_armorcheck_equipped_party(9) * 0.25);

if (global.chapter == 5 && i_ex(obj_flowery_enemy) && obj_flowery_enemy.partyistired == true)
    grazetpfactor -= 0.3;

if (grazetimefactor > 3)
    grazetimefactor = 3;

if (grazetpfactor > 3)
    grazetpfactor = 3;

if (grazesizefactor > 3)
    grazesizefactor = 3;

if (grazetimefactor < 0)
    grazetimefactor = 0;

if (grazetpfactor < 0)
    grazetpfactor = 0;

if (grazesizefactor < 0)
    grazesizefactor = 0;

image_xscale = grazesizefactor;
image_yscale = grazesizefactor;
angle = 0;

if (instance_exists(obj_heart))
{
    sizexoff = 0;
    sizeyoff = 0;
    x = obj_heart.x + 10 + sizexoff;
    y = obj_heart.y + 10 + sizeyoff;
}
