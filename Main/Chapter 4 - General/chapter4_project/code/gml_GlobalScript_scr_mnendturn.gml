function scr_mnendturn()
{
    ...
    if (techwon == 0)
    {
        ...
        for (i = 0; i < 3; i += 1)
        {
            ...
            if (global.chapter == 4 && i_ex(obj_titan_enemy) && obj_titan_enemy.gerson.sprite_index == spr_gerson_item)
                continue;
            
            if (global.char[i] != 0 && global.hp[hptarget] <= 0)
            {
            
                if (global.charweapon[3] != 15)
                    healamt = ceil(global.maxhp[hptarget] / 8);
                else
                    healamt = ceil(global.maxhp[hptarget] / 4);
                ...
                if (global.chararmor1[global.char[i]] == 200 || global.chararmor2[global.char[i]] == 200)
                    hasPin = true;
                
                if (hasPin == true && !pincheck[i])
                {
                    dmgwr.damage = global.maxhp[global.char[i]] - global.hp[global.char[i]];
