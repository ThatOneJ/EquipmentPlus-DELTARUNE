...
if (finishattacktimer > 0)
{
    ...
    if (finishattacktimer == 0)
    {
        ...
        if (cancelattack == 0)
        {
            ...
            if (global.chapter == 4)
            {
                // "object_index == obj_herokris" was here previously
                // Removed so that anyone equipping the BlackShard deals extra damage to the darkness enemies
                if ((scr_weaponcheck_equipped(1, 26) || scr_weaponcheck_equipped(1, 11)))
                {
            ...
            if (damage > 0)
            {
                ...
                if (i_ex(obj_titan_enemy) || i_ex(obj_titan_spawn_enemy) || i_ex(obj_jackenstein_enemy) || i_ex(obj_sound_of_justice_enemy))
                    scr_tensionheal( (round(points / 26)) * mult);
