...
if (con == 11 && !d_ex() && customcon == 1)
{
    ...
    if (scr_flag_get(853) > 0)
    {
        var _flash_step_two_ts = call_later(44, time_source_units_frames, _flash_step_two, false);
        var _flash_step_three_ts = call_later(48, time_source_units_frames, _flash_step_three, false);
        background.con = -1;
    }
...
if (con == 30 && customcon == 1 && !i_ex(obj_battlecontroller))
{
    ...
    _noroomweapon = noroom
    if defendonly
        scr_itemget_anytype(203,"armor")
    _noroomarmor = noroom
    c_waitcustom_end();
    c_sel(su);
    ...
    if defendonly
    {
        c_msgsetloc(0, "* (JusticeAxe was added to your WEAPONS.)/", "obj_dw_church_arena_slash_Step_0_gml_1193_0");
        c_sound_play(snd_item);
        c_msgsetloc(1, "* (JusticeShell was added to your ARMORS.)/%", "obj_dw_church_arena_slash_Step_0_gml_1193_0");
        }
    c_talk_wait();
